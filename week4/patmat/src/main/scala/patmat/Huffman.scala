...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    if (chars.isEmpty) List()
      ...
      val somePair = result.find(pair => pair._1 == chars.head)
      if (somePair != None)
        (somePair.get._1, somePair.get._2 + 1) :: result.filter(pair => pair._1 == chars.head)
      else
        (chars.head, 1) :: result
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.map(pair => Leaf(pair._1, pair._2)).sortWith((leaf1, leaf2) => leaf1.weight < leaf2.weight)

...
object Anagrams {
  ...
  /** The `dictionaryByOccurrences` is a `Map` from different occurrences to a sequence of all
    ...
    *
    * List(('a', 1), ('e', 1), ('t', 1)) -> Seq("ate", "eat", "tea")word
    *
    */
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def comb(pair: (Char, Int)): List[Occurrences] = pair match {
      case (_, 0) => List(List())
      case (c, n) => List(pair) :: comb((c, n - 1))
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    def subtractAux(occs: Occurrences, pair: (Char, Int)): Occurrences = {
      val (char, times) = pair
